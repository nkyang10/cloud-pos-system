...
- Run: `20260907-1207`
- Target feature: **stock items list + add item** (tiny POS system: track stock items)
- Input: no `NEXT-CYCLE.md` on this branch; the target feature itself is the approved input.

## Problem

This repo is a fresh playground for a tiny POS (point-of-sale) system. The first shippable
slice is the stock-tracking foundation: the cashier can see all stock items and add a new one.
This catalog is the base that later cycles (checkout, stock adjustments) will build on.
...
- **AC1** — `python run.py` starts a local server on port 8000 using only the Python standard
  library; `GET /` returns HTML listing stock items. Port is overridable via the `PORT` env var.
...
- **AC3** — Submitting the add form with a valid name (non-blank, unique case-insensitively),
  price (>= 0, at most two decimals), and quantity (>= 0 integer) creates the item, and the list
  shows it on the next view.
